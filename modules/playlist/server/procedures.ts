...
export const PlaylistRouter = createTRPCRouter({
    ...
    addVideo: protectedProcedure
        ...
        .mutation(async ({ input, ctx }) => {
            ...
            const [existingVideos] = await db.select().from(videos).where(eq(videos.id, videoId));
    ...
    getHistory: protectedProcedure
        ...
        .query(async ({ input, ctx }) => {
            ...
            const viewerVideoViews = db
                .$with('viewer_video_views')
                .as(db.select({ videoId: videoViews.videoId, viewedAt: videoViews.updatedAt }).from(videoViews).where(eq(videoViews.userId, userId)));
            const data = await db
    ...
    getLiked: protectedProcedure
        ...
        .query(async ({ input, ctx }) => {
            ...
            const viewerVideoReactions = db.$with('viewer_video_reactions').as(
                db
                    .select({ videoId: videoReactions.videoId, likedAt: videoReactions.updatedAt })
                    .from(videoReactions)
                    .where(and(eq(videoReactions.type, 'like'), eq(videoReactions.userId, userId))),
            );
            const data = await db
