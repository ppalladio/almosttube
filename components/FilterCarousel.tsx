        ...
        <div className="relative w-full">
            ...
            <Carousel setApi={setApi} opts={{ align: 'start', dragFree: true }} className="w-full px-12">
                <CarouselContent className="-ml-3">
                    ...
                    {isLoading &&
                        // length is equally to reset category length
                        Array.from({ length: 14 }).map((_, index) => (
                            <CarouselItem key={index} className="pl-3 basis-auto">
                                <Skeleton className="rounded-lg px-2 py-1 h-full text-sm w-[100px] font-semibold">&nbsp;</Skeleton>
                            </CarouselItem>
                        ))}
                    {!isLoading &&
                        data.map((item) => (
            ...
            <div
                className={cn(
                    'absolute right-12 top-0 bottom-0 w-12 bg-gradient-to-r from-white to-transparent pointer-events-none',
